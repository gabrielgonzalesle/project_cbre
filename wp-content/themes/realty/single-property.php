...
if ( $show_property == true ) {
	...
	<div id="property-layout-<?php echo $layout; ?>">
		...
		$type = '';
		if( $realty_theme_option['property-page-gallery-header'] == 'default' ) {
			...
			$header_meta_fields = $realty_theme_option['selected-property-meta-fields-header'];
			$property_header_meta = '';
			
			foreach ( $header_meta_fields as $header_field ) {
					
					switch ( $header_field ) {
						
						case 'estate_property_location' :
						
							$property_header_meta .= 'Location:';
							if( $property_location ) {
								
								foreach ( $property_location as $location ) { 
								
									$property_header_meta .= ' '. $location->name . ' ';
							    }
								$property_header_meta .= ' | ';
							}
						...
						case 'estate_property_type' :
							...
							$property_header_meta .= 'Type:';
							if( $property_type ) {
								
								foreach ( $property_type as $type ) { 
								
									$property_header_meta .= ' '. $type->name . ' ';
							    }
								$property_header_meta .= ' | ';
							}
						break;
						case 'estate_property_status' :
							...
							$property_header_meta .= 'Status:';
							if( $property_status ) {
								
								foreach ( $property_status as $status ) { 
								
									$property_header_meta .= ' '. $status->name . ' ';
							    }
								$property_header_meta .= ' | ';
							}
						...
						case 'estate_property_id' :
							...
							if ( $realty_theme_option['property-id-type'] == "custom_id" ) {
								...
								$property_header_meta .= '#' .' '. $property_id . ' | ';
							}
							else {
								
								$property_header_meta .= '#' .' '. $post->ID . ' | ';
							}
						break;
						...
						case 'estate_property_available_from' :
							...
							if( !empty($available_from_date) ) {
								...
								$property_header_meta .= 'From:' .' '. $available_from_date . ' | ';
							}
						...
						case 'estate_property_updated' :
							...
							if( !empty($last_updated_on) ) {
								...
								$property_header_meta .= 'Updated:' .' '. $last_updated_on . ' | ';
							}
						...
						case 'estate_property_views' :
							...
							$property_header_meta .= 'Views:' .' '. tt_get_property_views($post->ID) . ' | ';
						...
						case 'estate_property_size' :
							...
							if( ! empty($size) ){
							  ...
							  $property_header_meta .=  ' '. $size . ' ' . $size_unit . ' | ';
						...
						case 'estate_property_bathrooms' :
							...
							if( ! empty($bathrooms) ){
							  ...
							  $property_header_meta .=  'Bathrooms:' .' '. $bathrooms . ' | ';
						...
						case 'estate_property_bedrooms' :
							...
							if( ! empty($bedrooms) ){
							  ...
							  $property_header_meta .=  'Bedrooms:' .' '. $bedrooms . ' | ';
						...
						case 'estate_property_rooms' :
							...
							if( ! empty($rooms) ){
							  ...
							  $property_header_meta .=  'Rooms:' . ' ' . $rooms . ' | ';
						...
						case 'estate_property_garages' :
							...
							if( ! empty($size) ){
							  ...
							  $property_header_meta .=  'Garages:' .' '. $garages .' '. ' | ';
		...
		<div class="property-header-container primary-tooltips <?php echo $property_header_title_style . $property_header_media_class; ?>">
			...
			<div class="property-header">
				<h1 class="title">
					<span><?php _e(get_the_title()); ?></span>
				...
				<div class="meta"><?php _e($property_header_meta); ?></div>
				<div class="clearfix"></div>
				<?php if ( $property_status_update ) { echo '<div class="status-update">' . __($property_status_update ) . '</div>'; } ?>
			</div>
	...
	<div class="row">
      ...
      <?php if ( $property_image_location == 'begin' || $property_video_location == 'begin' ) {
	    ...
	    <?php } ?>
			...
			// Property Map
			if ( !empty ( $address ) && $realty_theme_option['show-single-property-map']  ) {
