...
function tt_featured_properties( $atts, $content = null ) {
	...
	if ( $style ) {
		...
		set_query_var( 'featured_listing_style', $style );
	}
	if ( $columns ) {
		
		set_query_var( 'featured_columns', $columns );
	}
	if ( !empty($location) ) {
		
		set_query_var( 'featured_location', $location );
	}
	if ( !empty($status) ) {
		
		set_query_var( 'featured_status', $status );
	}
	if ( !empty($type) ) {
		
		set_query_var( 'featured_type', $type );
	}
	if ( !empty($features) ) {
		
		set_query_var( 'featured_features', $features );
	}
...
function tt_property_listing( $atts, $content = null ) {
  ...
  <div id="property-items" class="show-compare<?php echo ' ' . $view; ?>"  data-view="<?php if ( isset( $listing_view ) && $view == '' ) { echo $listing_view; } else { echo $view; } ?>">
...
function tt_vertical_testimonials( $atts, $content = null ) {
	...
	$args_testimonials = array(
					...
					'orderby' => 'date',
					'order' => $order,
					'posts_per_page' => $count // If no gallery image selected, it will add all attachments
				);
	ob_start(); 
    $tt_testimonials = new WP_Query( $args_testimonials );
	while ( $tt_testimonials->have_posts() ) : $tt_testimonials->the_post();
	
	?>
	<div class="row vertical-testimonial testimonial-item">
   ...
   <style>
   .vertical-testimonial.testimonial-item{
	   margin: 35px;
   }
   </style>
<?php
...
add_shortcode('testimonials_vertical', 'tt_vertical_testimonials');
...
function tt_membership_packages( $atts, $content = null ) {
	...
	if ( $realty_theme_option['property-submission-type'] == 'membership' ) {
	  ...
	  <div id="property-items" class="show-compare subscriptions " data-view="grid-view" style="opacity: 1;"> 
			...
			<a href="<?php if ( is_user_logged_in() ) { echo get_permalink( get_the_ID() ); } else { echo "?login"; } ?>"><input type="button" value="<?php _e('Subscribe'); ?>" class="subscribe-button btn btn-primary btn-block form-control"></a>
		</li>
		<?php } ?>
