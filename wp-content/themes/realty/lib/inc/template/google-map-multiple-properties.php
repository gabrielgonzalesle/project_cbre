...
function initMarkers(map, markerData) {
	...
	<?php 
	global $realty_theme_option;
	$enable_listing_marker_sync = $realty_theme_option['enable-vertical-listing-marker-sync'];
	if( $enable_listing_marker_sync && is_page_template('template-map-vertical.php') ) { ?>
	jQuery(".property-item").on('mouseenter', function(){
			var id = jQuery(this).attr('id');
			newMarkers[id].setIcon(customAltIcon);
		}).on('mouseleave', function(){
			var id = jQuery(this).attr('id');
			newMarkers[id].setIcon(customIcon);      
	});  
	 <?php  } ?>  
