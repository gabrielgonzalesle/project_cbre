...
?>
<div <?php if(isset($property_counter)){ echo 'id="'.$property_counter.'"'; }?> class="property-item primary-tooltips<?php if ( $property_featured && $realty_theme_option['property-featured-listing-layout']!='featured-tag-view') { echo ' featured'; } echo ' ' . $realty_theme_option['property-listing-title-position']; ?>">
			...
			<figcaption>
				...
				<div class="property-excerpt">
					...
					if ( $property_type || $property_status || $property_location ) { 
						$property_meta = array();
					?>
					<div class="subtitle <?php if( $realty_theme_option['property-listing-show-status-type'] == false ){ echo 'hide'; } ?>">
						<?php 
						if ( $property_type ) { 
	...
	<div class="property-content">
		...
		$property_type_cbre = get_post_meta( $post->ID, 'acf-property-type', true );
		$aux = get_tag($property_type_cbre[0]);
		$property_status_cbre = get_post_meta( $post->ID, 'acf-property-status', true );
		$status_cbre = get_tag($property_status_cbre[0]);
		$status_cbre2 = get_tag($property_status_cbre[1]);
		$adicional = get_post_meta( $post->ID, 'especificacion_adicional', true );
		if ( $realty_theme_option['property-listing-type'] != "custom" && ( $size || $property_type_cbre || $property_status ) ) { ?>
		<div class="property-meta clearfix">
			<?php
			if ( ! empty( $aux->name ) ) { ?>
				<div>
					<div class="meta-title"><i class="fa fa-star"></i></div>
					<div class="meta-data" data-toggle="tooltip" title="<?php echo __( 'Tipo' , 'tt' ); ?>"><?php echo $aux->name;?></div>
				</div>
			<?php }
			if ( ! empty( $adicional ) ) { ?>
				<div>
					<div class="meta-title"><i class="fa fa-check-square-o" aria-hidden="true"></i></div>
					<div class="meta-data" data-toggle="tooltip" title="<?php echo __( 'información' , 'tt' ); ?>"><?php echo $adicional;?></div>
				</div>
			<?php }
			if ( ! empty( $status_cbre->name ) ) { 
				if(!empty($status_cbre->name) && !empty($status_cbre2->name)){
					$status_final=$status_cbre->name."/".$status_cbre2->name;
				}else{
					$status_final=$status_cbre->name;
				}
				?>
				<div>
					<div class="meta-title"><i class="fa fa-tag" aria-hidden="true"></i></div>
					<div class="meta-data" data-toggle="tooltip" title="<?php echo __( 'transacción' , 'tt' ); ?>"><?php echo $status_final; ?></div>
				</div>
			<?php }
			if ( ! empty( $size ) ) { ?>
				<div>
		...
		<div class="property-price">
          ...
          echo tt_paypal_payment_button( $post->ID);
				...
				?>
			<?php }	?>
			<div class="price-tag"><?php echo tt_property_price($post->ID); ?></div>
