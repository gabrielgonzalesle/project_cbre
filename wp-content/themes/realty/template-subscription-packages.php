	  ...
	  <div id="property-items" class="show-compare subscriptions " data-view="grid-view" style="opacity: 1;"> 
		...
		?>		
					...
					<div class="price-tag"><?php echo $realty_theme_option['currency-sign']; ?><?php _e($package_price); ?></div>
							...
							<a href="<?php if(is_user_logged_in()){ echo get_permalink($post->ID); } else{ echo "?login"; } ?>"><input type="button" value="<?php _e('Subscribe'); ?>" class="subscribe-button btn btn-primary btn-block form-control"></a>
						</li>
						<?php } ?>
	<?php endwhile;
	wp_reset_query();
