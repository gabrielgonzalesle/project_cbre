...
function tt_realty_scripts() {
	...
	wp_enqueue_style( 'custom_font', get_template_directory_uri() . '/assets/fonts/fonts.css', null, null );
	...
	wp_enqueue_script( 'imagemapjs', get_template_directory_uri() . '/assets/js/jquery.imagemapster.js', array( 'jquery' ), null, false ); 
...
if ( ! function_exists( 'tt_mapMarkers' ) ) {
	function tt_mapMarkers() {
		...
		if($realty_theme_option['enable-vertical-listing-marker-sync']) {
		...
		// check alertanive markers
      if ( !empty( $realty_theme_option['map-alternate-marker-property']['url']) && $realty_theme_option['enable-vertical-listing-marker-sync'] ) {
        ...
        <?php
            ...
            var customAltIcon = new google.maps.MarkerImage(
              '<?php echo get_template_directory_uri().'/lib/images/map-marker/map-marker-red-fat.png'; ?>',
                null, // size is determined at runtime
                null, // origin is 0,0
                null, // anchor is bottom center of the scaled image
...
if ( ! function_exists( 'tt_property_price' ) ) {
	function tt_property_price( $post_id = 0 ) {
		...
		$property_price = doubleval( get_post_meta( $post_id, 'estate_property_price', true ) );
		$property_price_prefix = get_post_meta( $post_id, 'estate_property_price_prefix', true );
		$property_price_suffix = get_post_meta( $post_id, 'estate_property_price_suffix', true );
...
function tt_author_permalinks() {
	...
	{
		$wp_roles->add_cap('agent','upload_files');
		$wp_roles->add_cap( 'agent', 'delete_posts' );
	}
...
add_filter( 'ajax_query_attachments_args', 'tt_show_users_own_media_library', 1, 1 );
...
if ( ! function_exists( 'tt_wpcf7_agent_admin_email' ) ) {
  ...
  function tt_wpcf7_agent_admin_email ($form) {
    ...
    $properites['mail']['body'] = $body_default . sprintf(__(''), '') . "\r\n\r\n" . __('Property: ') . get_permalink($postid) . "\r\n\r\n";
...
if ( ! function_exists( 'tt_property_submit_notification_admin_email' ) ) {
	...
	function tt_property_submit_notification_admin_email($post_id, $post, $update) {
		...
		if($realty_theme_option['property-submit-notification-email-recipient'] ) {
			...
			$admin_message  = sprintf( __('A new property #%1$s "%2$s" has been added'), $post_id , $post->post_title ) . "\r\n";
			$admin_message .= sprintf( __('Author : %1$s'), $post->display_name ) . "\r\n";
			$admin_message .= sprintf( __('E-mail : %s'), $post->user_email ) . "\r\n";
			$admin_message .= sprintf( __('To have a look, visit: %s'),  get_settings('siteurl').'/wp-admin/post.php?action=edit&post='.$post_id) . "\r\n";
			
			$admin_message .= get_settings('siteurl') . "/wp-admin/edit.php?post_type=property\r\n";
		
			$subject = sprintf( __('[%1$s] New Property: "%2$s"'), get_settings('blogname'), $post->post_title );
