...
<?php if ( $show_footer ) { ?>
<footer id="footer">
	<img style="width:100%" src="<?php echo get_site_url(); ?>/wp-content/images/Footer.jpg">
    <?php if ( ! $hide_footer_widgets) { ?>
        ...
        <div id="footer-top">
            <div class="container">	
                ...
                ?>
                <div class="col-sm-3">
                	<img src="<?php echo get_site_url(); ?>/wp-content/images/logo/logo_footer.png">
                </div>
                </div>			
            </div>
        </div>
...
(function($){
	$("#dep").change(function(){
		var id = $(this).val();
		var url   = "<?php echo get_bloginfo('wpurl') ?>/wp-content/plugins/ajax_test/clear_cache.php";
		$.post( 
              url,
              { id: id },
              function(data) {
              	var result=$.parseJSON(data);
                $("#prov").html(result.out);
              }
           );
	});
	$("#prov").change(function(){
		var id = $(this).val();
		var url   = "<?php echo get_bloginfo('wpurl') ?>/wp-content/plugins/ajax_test/get_distritos.php";
		$.post( 
              url,
              { id: id },
              function(data) {
              	var result=$.parseJSON(data);
                $("#dist").html(result.out);
              }
           );
	});
})(jQuery);  
